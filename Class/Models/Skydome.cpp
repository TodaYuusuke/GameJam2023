...
Microsoft::WRL::ComPtr<ID3D12Resource> Skydome::CreateBufferResource(const Microsoft::WRL::ComPtr<ID3D12Device>& device, size_t sizeInBytes) {
	HRESULT hr;
	// 頂点リソース用のヒープの設定
...
void Skydome::CreateMaterialResource() {
...
void Skydome::CreateWvpResource() {
	// 1つ分のサイズを用意する
...
void Skydome::Initialize() {
	// モデルを読み込み
	modelData_ = ObjManager::GetInstance()->GetObjModelData()[skydome];
...
void Skydome::Draw() {
		...
		// DescriptorTableの設定
		DirectXCommon::GetInstance()->GetCommandList()->SetGraphicsRootDescriptorTable(2, TextureManager::GetInstance()->GetTextureSrvHandleGPU()[SKYDOME]);
...
void Skydome::ApplyGlobalVariables() {
	GlobalVariables* globalVariables = GlobalVariables::GetInstance();
	const char* groupName = "Skydome";
...
void Skydome::ImGuiAdjustParameter() {
